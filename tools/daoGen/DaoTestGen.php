...
// dao path
defined('DAO_PATH') or define('DAO_PATH', __DIR__ . DIRECTORY_SEPARATOR . 'src' . DIRECTORY_SEPARATOR);
// phpunit path
defined('PHPUNIT_PATH') or define('PHPUNIT_PATH', __DIR__ . DIRECTORY_SEPARATOR . 'tests' . DIRECTORY_SEPARATOR);
defined('PHPUNIT_DAO_PATH') or define('PHPUNIT_DAO_PATH',
    PHPUNIT_PATH . DIRECTORY_SEPARATOR . 'unit' . DIRECTORY_SEPARATOR . 'dao' . DIRECTORY_SEPARATOR);
...
defined('PHPUNIT_DEMO_FILE') or define('PHPUNIT_DEMO_FILE', __DIR__ . DIRECTORY_SEPARATOR . 'DemoDaoTest.php.php');
...
if ($handle = opendir(DAO_PATH)) {
    echo "Generate PHPUnit Test Directory: \n";

    /* 这是正确地遍历目录方法 */
    while (false !== ($dir = readdir($handle))) {
        if (!in_array($dir, ['.', '..']) && is_dir(DAO_PATH . DIRECTORY_SEPARATOR . $dir)) {
            $PHPUnitPath = PHPUNIT_DAO_PATH . $dir;
            if (!file_exists($PHPUnitPath)) {
                mkdir($PHPUnitPath);
                echo "Generate Directory : $PHPUnitPath \n";
            }

            if ($handleSon = opendir(DAO_PATH . DIRECTORY_SEPARATOR . $dir)) {
                while (false !== ($file = readdir($handleSon))) {
                    $fileName = str_replace('.php', 'Test.php', $file);
                    $daoFile = DAO_PATH . DIRECTORY_SEPARATOR . $dir . DIRECTORY_SEPARATOR . $file;
                    $PHPUnitTestFile = $PHPUnitPath . DIRECTORY_SEPARATOR . $fileName;
                    if (!file_exists($PHPUnitTestFile)) {
                        $tblName = getFixtures($daoFile);
                        $daoFunctions = getDaoClassFunctions($dir, $file);
...
function generateFile($tblName, $file, $dir, $functions)
{
    ...
    $fileInfo = str_replace('DemoDaoTest', $fileName, $fileInfo);
    $fileInfo = str_replace('DemoDir', $dir, $fileInfo);
    $fileInfo = str_replace('Demo', str_replace('DaoTest', '', $fileName), $fileInfo);
...
function getDaoClassFunctions($dir, $file)
{
    $parentFunctions = [
        "_rules",
        "__construct",
    ...
    $classFuns = get_class_methods('\\src\\' . $dir . '\\' . pathinfo($file, PATHINFO_FILENAME));
